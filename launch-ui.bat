...
:: ----------------------------------------------------------
:: Add hosts entry for amplify-qea (once)
:: ----------------------------------------------------------
findstr /c:"amplify-qea" C:\Windows\System32\drivers\etc\hosts >nul 2>&1
if %errorlevel% neq 0 (
    echo [INFO] Adding 'amplify-qea' to hosts file...
    echo 127.0.0.1  amplify-qea>> C:\Windows\System32\drivers\etc\hosts 2>nul
    if %errorlevel% neq 0 (
        echo [WARN] Could not update hosts file. Right-click launch-ui.bat and choose
        echo        "Run as administrator" to enable the amplify-qea URL.
...
echo [INFO] Starting server at http://amplify-qea:7777
